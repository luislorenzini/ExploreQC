...
[fdir, fname, ~ ]= fileparts(ExploreQC);
...
if bStructural
    ...
    for iSub = 1:length(Subjects)
        ...
        T1Path = fullfile(SubjDir, config.PE_properties{strcmp(config.ParameterExtractionModule, 'StructIm')});
        c1T1Path = fullfile(SubjDir, config.PE_properties{strcmp(config.ParameterExtractionModule, 'GMim')});
        c2T1Path = fullfile(SubjDir, config.PE_properties{strcmp(config.ParameterExtractionModule, 'WMim')});
        c3T1Path = fullfile(SubjDir, config.PE_properties{strcmp(config.ParameterExtractionModule, 'CSFim')});
        yfile =  fullfile(SubjDir, config.PE_properties{strcmp(config.ParameterExtractionModule, 'Struct_Y_file')});
        ...
        if domainconfig{strcmp(domainconfig.ScanType_Domain, 'Structural_IT'),2}
        ...
        if domainconfig{strcmp(domainconfig.ScanType_Domain, 'Structural_General'),2}
...
if bFunctional
    ...
    for iSub = 1:length(Subjects)
        ...
        SubjDir = fullfile(AnalysisDir, Subject);
        FuncFold = fullfile(SubjDir, 'func');
        ...
        c2T1Path = fullfile(SubjDir, config.PE_properties{strcmp(config.ParameterExtractionModule, 'WMim')});
        c3T1Path = fullfile(SubjDir, config.PE_properties{strcmp(config.ParameterExtractionModule, 'CSFim')});
        yfile =  fullfile(SubjDir, config.PE_properties{strcmp(config.ParameterExtractionModule, 'Func_Y_file')});
        % Check if file exist
        ...
        %SPATIAL DOMAIN
        if domainconfig{strcmp(domainconfig.ScanType_Domain, 'Functional_Spatial'),2}
            NoisIT = xQC_Noise_IT(boldnii, c1T1Path, c2T1Path, SubjDir, AtlasDir, x, yfile);
            
            QC.(sSubject).Functional.Spatial = NoisIT.IT;
            
            QC.(sSubject).Functional.Spatial.GhostToSignal = xQC_GhostToSignal(boldnii, GhostTemplate);
        end
        % TEMPORAL DOMAIN
        if domainconfig{strcmp(domainconfig.ScanType_Domain, 'Functional_Temporal'),2}
            %NoisIT = XQC_Noise_IT(boldnii, c1T1Path, c2T1Path, SubjDir, AtlasDir, x, yfile);
            ...
            xASL_spm_reslice(boldnii,c1T1Path , [], [], [], fullfile(SubjDir, 'tmp_GM_mask.nii'), 0)
            xASL_spm_reslice(boldnii,c2T1Path , [], [], [], fullfile(SubjDir, 'tmp_WM_mask.nii'), 0)
            
            tSNR = xASL_qc_temporalSNR(boldnii,{fullfile(SubjDir, 'tmp_GM_mask.nii') fullfile(SubjDir, 'tmp_WM_mask.nii')});
            
            xASL_delete(fullfile(SubjDir, 'tmp_GM_mask.nii'))
            xASL_delete(fullfile(SubjDir, 'tmp_WM_mask.nii'))
            
            QC.(sSubject).Functional.Temporal = tSNR;
            QC.(sSubject).Functional.Temporal.GlobalCorrelation = xQC_Global_correlation(boldnii, c1T1Path, SubjDir);
        end
        
        % MOTION DOMAIN
        if domainconfig{strcmp(domainconfig.ScanType_Domain, 'Functional_Motion'),2}
            %this is the path to the motion files according to ExploreASL
            %structure
    ...
    if bDiffusion
        ...
        for iSub = 1:length(Subjects)
            ...
            if domainconfig{strcmp(domainconfig.ScanType_Domain, 'Diffusion_Motion'),2}
                [EddyQC] = xQC_dwi_EddyMotion(SubjDir, Subject, c1T1Path, c2T1Path, c3T1Path, TopUp_Path);
            ...
            if domainconfig{strcmp(domainconfig.ScanType_Domain, 'Diffusion_Noise'),2}
                [Noise_mean , Noise_SD ] = xQC_dwi_Noise(SubjDir, 'dwi_run-1_dwi.nii', 0 , 0) ; % don't save the noise image for now
